...
- Call codex for review and suggestions, and agent design + google adk implementation
- 在写任何代码前，先**彻底阅读**现有文档与 mockups，确认：
  - 产品目标：Puzzle app、四象限 canvas、角落 puzzle hubs、中心问题（central questions）。
  - 当前 scope：不要自行扩展无关 feature。
- for each phase, 输出一个简短但**结构化的 plan**（放在 `docs/PLAN-<date>.md`）：
...
- update the building.md when the things have been finished
