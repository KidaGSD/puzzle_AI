...
export const PuzzlePiece: React.FC<PuzzlePieceProps> = ({ data }) => {
  ...
  const textLayout = useMemo(() => {
    ...
    // Use 85% of the inscribed rectangle to add padding
    const containerWidth = textBoundingBox.width * 0.85;
    const containerHeight = textBoundingBox.height * 0.85;

    // Adaptive font size based on container size
    const fontSize = isTall
      ? Math.max(7, Math.min(10, containerHeight / 8))
      : Math.max(8, Math.min(12, containerWidth / 15));

    // Line height: tighter for tall shapes
    const lineHeight = isTall ? 1.15 : 1.25;

    // Max lines based on shape
    const maxLines = isTall ? Math.min(5, Math.floor(containerHeight / (fontSize * lineHeight))) : 3;
