                        ...
                        <AnimatePresence mode='wait'>
                         {currentShape.map((cell, idx) => (
                             <motion.div 
                                ...
                                className={clsx(
                                    "absolute rounded-lg shadow-2xl transition-colors duration-200 backdrop-blur-md",
                                    !isValid && "bg-red-500/80 ring-2 ring-red-400",
                                    isValid && "ring-1 ring-white/50"
                                )}
                                style={{
                                    width: CELL_SIZE - 4,
                                    height: CELL_SIZE - 4,
                                    background: isValid 
                                        ? `linear-gradient(135deg, ${color}F2, ${color}66)` 
                                        : undefined,
                                    left: cell.x * CELL_SIZE + 2,
                                    top: cell.y * CELL_SIZE + 2,
                                }}
                             >
                                 {isValid && <div className="absolute inset-0 rounded-lg bg-gradient-to-br from-white/30 to-transparent pointer-events-none" />}
                                 {idx === 0 && isValid && (
                                    <motion.div 
                                        key={currentLabel}
                                    ...
                                    >
                                        <span className="text-white font-bold text-[11px] uppercase tracking-wider text-center drop-shadow-md leading-none">
                                            {currentLabel}
                                        </span>
                                    </motion.div>
                                 )}
