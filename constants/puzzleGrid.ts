...
export const CENTER_CARD_WIDTH = 6;
...
/**
 * Get shape based on attachment order (1-8, then stays at 8)
...
export const getSequentialShape = (attachmentIndex: number): { x: number; y: number }[] => {
  // Shapes 1-8, after 8 stays at shape 8
  const shapeIndex = Math.min(attachmentIndex, ALL_SHAPES.length - 1);
  return ALL_SHAPES[shapeIndex];
};
